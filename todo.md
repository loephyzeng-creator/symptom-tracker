...
- [x] 打卡日历日详情中展示备注
- [x] 历史记录展开详情中展示备注
- [x] 历史页面视图切换图标（列表/日历/药品）间距拉大

## 头痛发作等级 + 止疼药功能
- [x] 将"是否剧烈头痛"改为"头痛发作"等级选择（无/轻微/明显/严重）
- [x] 添加"是否服用止疼药"选项
- [x] 实现止疼药30天内不超过10天的提醒逻辑
- [x] 更新统计/历史/报告中相关字段的展示
- [x] 在止疼药开关旁常驻显示30天累计服用天数

## 止疼药阈值可配置
- [x] 数据库添加止疼药报警阈值字段（默认10天）
- [x] 后端API：获取/更新阈值设置
- [x] 设置页面添加阈值配置入口
- [x] 记录页面使用用户自定义阈值替代硬编码的10

## 报告集成到历史tab
- [x] 将报告功能集成到历史tab中（作为子视图或按钮入口）
- [x] 移除底部导航的报告tab

## 三项功能增强
- [x] 历史tab视图切换图标添加文字标签（列表/日历/用药/报告）
- [x] 统计tab增加头痛发作频率和止疼药使用趋势图表
- [x] 报告视图添加AI智能分析入口（一键生成症状趋势解读和就诊建议）

## Bug修复
- [x] 修复止疼药选项保存失败的问题（painkillerTaken字段未正确保存，天数未累加）
- [x] 修复止疼药累计天数未正确显示（3月20日保存了止疼药，但3月21日累计未显示1天）
- [x] 修复止疼药累计天数保存后不刷新（需要invalidate painkillerUsage查询缓存）
- [x] 在打卡日历中标记服用止疼药的日期

## 打卡日历增强
- [x] 日详情展示头痛发作等级 + 止疼药使用关联
- [x] 日历上方增加本月止疼药使用天数汇总统计
- [x] 长按日历某天快速标记/取消止疼药

## 止疼药分析与可视化增强
- [x] AI分析加入止疼药使用与头痛等级的关联分析（识别用药效果）
- [x] 用药tab添加“本月止疼药趋势”迷你折线图（按周展示使用频率变化）
- [x] 日历上止疼药标记更明显直观（加大标记、增加背景色或边框）

## Bug修复
- [x] 3月20日头痛数据未在统计中展示（原因：HeadachePainkillerChart使用了错误的字段名headacheAttack，应为severeHeadache）

## AI分析报告导出PDF
- [x] 前端：AI分析结果页面添加“导出PDF”按钮
- [x] 前端：将Markdown分析报告转换为可打印的PDF格式（含标题、日期、格式化内容）
- [x] 前端：PDF包含患者信息头、分析日期、完整报告内容

## 止疼药品牌剂量提示
- [x] 前端：记录保存时检测是否标记了止疼药
- [x] 前端：标记止疼药时弹出Dialog提示输入品牌和剂量
- [x] 后端/数据库：保存止疼药品牌和剂量信息

## UI优化
- [x] 头痛发作卡片只显示发作天数，去掉“/X天”总天数
- [x] 修复用药频率统计图Y轴药品名称被截断的问题（动态计算Y轴宽度，最小100px）
